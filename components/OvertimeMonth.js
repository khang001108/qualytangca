...
export default function OvertimeMonth({
  ...
  // 🔥 TÍNH THỨ CỦA NGÀY 1 — ĐỂ CĂN NGÀY GIỐNG WINDOWS
  const firstDay = dayjs(
    `${selectedYear}-${selectedMonth}-01`
  ).day(); // 0 = CN, 1 = T2, ...

  // Layout lịch của cậu: T2 → T3 → T4 → T5 → T6 → T7 → CN
  // Convert về index tương ứng
      ...
      {open && (
        ...
        >
          ...
          <div className="grid grid-cols-7 gap-1 text-center text-xs">
            ...
            {/* 🔥 Ô TRỐNG ĐẦU THÁNG — giúp ngày 1 vào đúng thứ */}
