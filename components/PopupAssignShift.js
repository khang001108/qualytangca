...
export default function PopupAssignShift({
    ...
    const handleMouseEnter = (day) => {
        if (!dragState.current.isSelecting || !dragState.current.selectingShift) return;

        if (!dragState.current.dragged) {
            dragState.current.dragged = true;
            const start = dragState.current.pressDay;
            setAssignMap((prev) => {
                const next = { ...prev };
                if (start) next[start] = dragState.current.selectingShift;
                next[day] = dragState.current.selectingShift;
                return next;
            });
            return;
        }

        setAssignMap((prev) => ({ ...prev, [day]: dragState.current.selectingShift }));
    };

    const handleMouseUp = (day, e) => {
        e.preventDefault();
        e.stopPropagation();
        ...
        // 🔄 Toggle ưu tiên ca hiện tại (shiftType)
        setAssignMap((prev) => {
            const cur = prev[pressDay] ?? null;

            // Nếu đang chọn "Ca ngày" thì ưu tiên theo thứ tự: day → night → null
            // Nếu đang chọn "Ca đêm" thì đảo ngược: night → day → null
            let next = null;
            if (shiftType === "day") {
        ...
        try {
            ...
            for (const dayStr of selectedDays) {
                ...
                for (const m of members) {
                    ...
                    // 🔹 Ca sớm/muộn dựa theo earlyShift
                    const shiftStart =
                        type === "day"
                            ? m.earlyShift
                                ? "07:00"
            ...
            // 🔹 Cập nhật shiftStart toàn bộ member theo earlyShift
            for (const m of members) {
                if (!m?.id) continue;
                const newShiftStart =
                    newShift === "Ca ngày"
            ...
            alert("✅ Đã lưu và đồng bộ phân ca (có ca sớm/muộn) về danh sách nhân viên!");
            onSuccess?.();
        } catch (err) {
            ...
            >
                ...
                <div className="flex justify-center gap-3 mb-4">
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg border ${shiftType === "day"
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-lg border ${shiftType === "night"
                ...
                <p className="text-xs text-gray-500 mb-4 text-center">
                    ...
                    để chọn nhanh. Màu được lưu lại vĩnh viễn trong Firestore.
                </p>
