  ...
  async function parseText(rawText, mode = "checkin") {
    ...
    try {
      ...
      for (let raw of lines) {
        ...
        function floorToHour(min) {
          if (min == null) return null;
          return Math.floor(min / 60) * 60;
        }

        const leaveMatch = trailing.match(/(\d+)\s*h/i);
        const leaveHours = leaveMatch ? Number(leaveMatch[1]) : 0;
        const hasFormalLeaveToken = /事假|nghỉ/i.test(trailing);

        if (leaveHours && leaveHours > 0) {
          const leaveInfo = { leaveHours };

          await upsertShiftSchedule(dateStr, member, {
            note: `${trailing}`.slice(0, 200),
            partialLeave: {
              hours: leaveHours,
              detectedAt: minutesToHHMM(minutesOfDay),
              mode,
            },
          });

          if (mode === "checkin") {
            const presumShiftRec =
              shiftRec || (await getShiftSchedule(dateStr, member.id));
            const presumIsNight =
              (presumShiftRec &&
                String(presumShiftRec.shift).toLowerCase().includes("đêm")) ||
              String(member.shift || "")
                .toLowerCase()
                .includes("đêm") ||
              !(minutesOfDay >= 4 * 60 && minutesOfDay < 12 * 60);

            const presumCfg = presumIsNight ? shiftNight || {} : shiftDay || {};
            const baseLenHourMin = floorToHour(
              timeToMinutes(
                presumCfg.lenCaSomKetThuc ??
                  presumCfg.lenCaMuonKetThuc ??
                  "00:00"
              )
            );
            const baseStart = Number.isFinite(baseLenHourMin)
              ? baseLenHourMin
              : Math.floor((minutesOfDay - leaveHours * 60) / 60) * 60;

            const leaveEnd = baseStart + leaveHours * 60;
            const breakEnd = leaveEnd + 60;

            if (minutesOfDay >= breakEnd - 30 && minutesOfDay <= breakEnd) {
              await upsertShiftSchedule(dateStr, member, {
                lenCa: minutesToHHMM(breakEnd),
                checkinType: "som",
                partialLeave: { hours: leaveHours, appliedAs: "checkin" },
                note: `${trailing}`.slice(0, 200),
              });
              updated++;
              continue;
            ...
            if (minutesOfDay >= leaveEnd && minutesOfDay <= leaveEnd + 30) {
              await upsertShiftSchedule(dateStr, member, {
                xuongCa: minutesToHHMM(leaveEnd),
                partialLeave: { hours: leaveHours, appliedAs: "checkout" },
                note: `${trailing}`.slice(0, 200),
              });
              updated++;
              continue;
